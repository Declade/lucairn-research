...
Overall recall on the 500-row Measurement B subset: **79.0 %** (8 916 TP / 11 285 ground-truth annotations). The strongest weak category is `DATE` (53.9 %) and accounts for the majority of all missed annotations (1 816 / 2 369 ≈ 77 % of FN). Source data: [`papers/paper-1-healthcare/raw-results/paper1-AFTER-500row-20260522T080037Z.ndjson`](./papers/paper-1-healthcare/raw-results/paper1-AFTER-500row-20260522T080037Z.ndjson).
...
```mermaid
flowchart LR
    A[Customer doc] --> L1[L1: regex]
    L1 --> L2[L2: deny-list]
    L2 --> L3[L3: PII Shield<br/>Qwen 2.5 fine-tunable]
...
The bar charts above measure L1+L2+L3 recall against the published HIPAA Safe Harbor (Paper 1) and operational GLBA NPI (Paper 2) enumerations. L4 (the reid-guard adversarial scorer) is benchmarked separately — see the L4 pressure-test handover in the dual-sandbox-architecture repo. A static SVG version of this diagram for non-GitHub renderers lives at [`docs/figures/methodology-pipeline.svg`](./docs/figures/methodology-pipeline.svg).
...
The bar charts and pipeline diagram referenced in the README are generated by `scripts/build-figures.ts` from the data already committed under `papers/`. To re-render them after updating measurements:
...
- `paper1-recall-per-category.svg` — Paper 1, computed live from `papers/paper-1-healthcare/raw-results/paper1-AFTER-500row-20260522T080037Z.ndjson`.
